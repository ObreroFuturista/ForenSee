...
def download_images(taxon_id, save_directory, place_ids, per_page=100, max_pages=None, research_only=False, num_threads=10, allowed_licenses=[]):
            ...
            while True:
                ...
                for result in data["results"]:
                    ...
                    for photo in result["photos"]:
                        # Verifica si la licencia de la imagen está en la lista de licencias permitidas
                        if allowed_licenses and photo["license_code"] not in allowed_licenses:
                            continue
                        ...
                        # Agrega el código de licencia al nombre del archivo
                        license_suffix = f"_{photo['license_code']}" if photo['license_code'] else ""
                        save_path = os.path.join(save_directory, f"{genus}_{image_id}{research_suffix}{license_suffix}.jpg")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--allowed_licenses", nargs='+', default=["cc0", "cc-by-nc", "cc-by-sa"], help="Licencias permitidas para las imágenes a descargar (predeterminado: CC0, CC BY-NC, CC BY-SA)")

    args = parser.parse_args()

    download_images(args.taxon_id, args.save_directory, args.place_ids, per_page=args.per_page, max_pages=args.max_pages, research_only=args.research_only, num_threads=args.num_threads, allowed_licenses=args.allowed_licenses)
